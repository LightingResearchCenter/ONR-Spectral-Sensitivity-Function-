...
plot_allSubjects <- function(plotData, outcomeMeasure, color){
  ...
  plotData$sub_cond <- paste(plotData$Subject_id, plotData$condition, sep = "_")
  #plotData$block <- as.factor(plotData$block)
  
  if(outcomeMeasure == "fp"){
    no_only <- subset(plotData, display == "no_go")
    
    meanplotData <-  summarySE(data = no_only, measurevar = "false_positive", groupvars = c("Subject_id", "color", "condition", "block", "sub_cond"))
    ...
    subPlot1 <- ggplot(meanplotData, aes_string(x="block", y="false_positive", colour = "condition", group = "sub_cond")) +
      ...
      geom_segment(aes(x = .5,xend=.7,y=figSpecsList[2],yend=figSpecsList[2]), color = "red", linetype = "dashed",  size = .4) + 
      annotate("text", x = .8, y = figSpecsList[2]+(figSpecsList[2]*.07), label = paste("mu : ", figSpecsList[2], sep = ""), parse = TRUE, color = "red", size = fontSize) +
      
      
      geom_segment(aes(x = 4.1,xend=4.3,y=figSpecsList[3],yend=figSpecsList[3]), color = "red", linetype = "dashed", size = .4) + 
      annotate("text", x = 4, y=figSpecsList[3]+(figSpecsList[3]*.07), label = paste("M : ", figSpecsList[3], sep = ""), parse = TRUE, color = "red", size = fontSize) +
      
      
      theme(axis.title.x=element_blank(), legend.position="none", plot.title = element_text(hjust = 0.5))+
      ggtitle(titleText)+
      scale_x_continuous(expand = c(0, 0), limits = c(0,5), breaks = c( 1, 2, 3, 4)) +
      #xlim(.5, 4.5) +
      #scale_y_continuous(expand = c(0, 0), limits = c(0,15))+
      facet_wrap(~ Subject_id*condition,  ncol=4, scales = "free")+
  ...
  if(outcomeMeasure == "miss"){
    ...
    meanplotData <-  summarySE(data = go_only, measurevar = "miss", groupvars = c("Subject_id", "color", "condition", "block", "sub_cond"))
    ...
    subPlot1 <- ggplot(meanplotData, aes_string(x="block", y="miss", colour = "condition", group = "sub_cond")) +
      geom_line(colour =  "black")+
  ...
  if(outcomeMeasure == "rt"){
    ...
    meanplotData <-  summarySE(data = rt_gng, measurevar = "response_time", groupvars = c("Subject_id", "color", "condition", "block", "sub_cond"))
    ...
    subPlot1 <- ggplot(meanplotData, aes_string(x="block", y="response_time", colour = "condition", group = "sub_cond")) +
      geom_line(colour =  "black")+
  ...
  ##Add raw data plot in here
  
  if(outcomeMeasure == "raw"){
    plotData$response_time2 <- plotData$response_time
    plotData$response_time2 <- ifelse(plotData$miss == 1, 1.5,
                                      ifelse(plotData$false_positive == 1, -.5,
                                             ifelse(plotData$response_detail == "CORRECT INHIBIT", 0,
                                                    plotData$response_time2)))
    ...
    subPlot1 <- ggplot(plotData, aes_string(x="trial_num", y="response_time2", colour = "accuracy", shape = "response_detail")) +
      geom_point( size = .7)+
      labs(y = unitString)+
      ...
      facet_wrap(~ Subject_id*condition*block,  ncol=4, scales = "free")+
      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
            panel.background = element_blank(), axis.line = element_line(colour = "black"), 
            plot.title = element_text(size = 30), strip.text = element_text(size=11))
      
    
    subPlot1 <- subPlot1 +   scale_colour_manual(values=c("black", "red1")) 
...
if(FALSE){
  ...
  miss_blue <- plot_allSubjects(performance, 'miss', "blue" )
  
  miss_blue <- plot_allSubjects(performance, 'raw', "blue" )
...
saveGraph <- function(dayNight, colorList, PlotData ){
  ...
  for(i in 1:length(figure_list)){
    ...
    if(tolower(dayNight) == "day"){
      ...
      pdfFileName <- paste(figure_list[i], "2_day.pdf")
    }
    if(tolower(dayNight) == "night"){
      dir <- "//root/projects/ONR-EEG-BAA16_001/REPORTS/GRAPHS/SUBJECT/NIGHT/GNG/"
      pdfFileName <- paste(figure_list[i], "2_night.pdf")
    }
    if(curr[[1]][2] == "raw"){
      ggsave(paste(dir, pdfFileName, sep = ""), dpi = 250, width = 14.5, height = (ceiling(nSubs/3))*36, units = "in", limitsize = FALSE)
      
    }
    else{
      ggsave(paste(dir, pdfFileName, sep = ""), dpi = 250, width = 14.5, height = (ceiling(nSubs/3))*9, units = "in", limitsize = FALSE)
